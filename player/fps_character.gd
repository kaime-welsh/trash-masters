...
func _snap_down_to_stairs_check() -> void:
	...
	var floor_below: bool = (
		stairs_below_raycast.is_colliding()
		and not is_surface_too_steep(stairs_below_raycast.get_collision_normal())
	)
...
func _snap_up_stairs_check(delta) -> bool:
	...
	if (
		...
		if (
			...
			return false
		stairs_ahead_raycast.global_position = (
		...
		if (
			stairs_ahead_raycast.is_colliding()
			and not is_surface_too_steep(stairs_ahead_raycast.get_collision_normal())
		):
			_save_camera_pos_for_smoothing()
...
func _handle_crouch(delta) -> void:
	...
	head.position.y = move_toward(
		head.position.y, -crouch_height if is_crouched else 0.0, 7.0 * delta
	)
	collision_shape.shape.height = (
		_original_capsule_height - crouch_height if is_crouched else _original_capsule_height
	)
	...
	for i in get_slide_collision_count():
		...
		if c.get_collider() is RigidBody3D:
			...
			var velocity_diff_in_push_dir = (
				self.velocity.dot(push_dir) - c.get_collider().linear_velocity.dot(push_dir)
			)
